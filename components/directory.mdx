...
Some of its important fields are:
...
<Tabs groupId="shell">
  <Tab value="System shell">```shell dagger -c 'debug .' ```</Tab>
  <Tab value="Dagger Shell">
    ```shell title="First type 'dagger' for interactive mode." debug . ```
  </Tab>
  <Tab value="Dagger CLI">```shell dagger call debug --source=. ```</Tab>
</Tabs>

You can also list all files, recursively to check it more deeply:

<Tabs groupId="shell">
  <Tab value="System shell">```shell dagger -c 'debug . | glob "**/*"' ```</Tab>
  <Tab value="Dagger Shell">
    ```shell title="First type 'dagger' for interactive mode." debug . | glob
    "**/*" ```
  </Tab>
  <Tab value="Dagger CLI">
    ```shell dagger call debug --source=. glob --pattern="**/*" ```
  </Tab>
</Tabs>
...
<Tabs groupId="shell">
  <Tab value="System shell">```shell dagger -c 'debug . | terminal' ```</Tab>
  <Tab value="Dagger Shell">
    ```shell title="First type 'dagger' for interactive mode." debug . |
    terminal ```
  </Tab>
  <Tab value="Dagger CLI">
    ```shell dagger call debug --source=. terminal ```
  </Tab>
</Tabs>
...
<Tabs groupId="shell">
  <Tab value="System shell">
    ```shell dagger -c 'debug . | export audit' ```
  </Tab>
  <Tab value="Dagger Shell">
    ```shell title="First type 'dagger' for interactive mode." debug . | export
    audit ```
  </Tab>
  <Tab value="Dagger CLI">
    ```shell dagger call debug --source=. export --path=audit ```
  </Tab>
</Tabs>
