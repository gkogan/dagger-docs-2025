...
import Directory from '/snippets/core-types/directory.mdx';
...
```go Go  icon="golang" highlight={12} wrap
package main
...
```python Python icon="python" highlight={11} wrap
...
```typescript TypeScript icon="js" highlight={7} wrap
...
```php PHP icon="php" highlight={16} wrap
<?php
...
```shell Shell icon="terminal" wrap
...
<ShellTabs 
systemShell="dagger -c 'debug .'"
daggerShell="debug ."
daggerCli="dagger call debug --source=."
/>

You can open the directory in an interactive terminal to inspect the filesystem:

<ShellTabs
systemShell="dagger -c 'debug . | terminal'"
daggerShell="debug . | terminal"
daggerCli="dagger call debug --source=. terminal"
/>

You can export the filtered directory to your host and check it with local tools:

<ShellTabs
systemShell="dagger -c 'debug . | export audit'"
daggerShell="debug . | export audit"
daggerCli="dagger call debug --source=. export --path=audit"
/>
